sap.ui.define([
...
], function (BaseController, formatter, MessagePopover, MessagePopoverItem, Link, JSONModel) {
  "use strict";
  var oLink = new Link({
    text: "",
    href: "",
  ...
  return BaseController.extend("zprelimdoc.controller.DocDetail", {
    formatter: formatter,
    hasChanges: false,
    oClonedObject: {},
    ...
    onCurrentModelSave: function (channelId, eventId, data, skipCheck, antos) {
      ...
      if ($.grep(mdocArray, function (e) { return e.DocContent == that.jModel.getData().DocContent }).length == 0) {
        ...
        $.each(mdocArray, function (e) {
          if (e.DocContent == that.jModel.getData().DocContent) { e = JSON.parse(JSON.stringify(that.jModel.getData())); }
        })
      }
